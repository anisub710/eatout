...
server <- function(input, output){
  ...
  reactive({
    ...
    data.categories <- group_by(data.categories, Food_Category) %>% summarise('Count' = n()) %>% arrange(desc(Count))
    data.categories <- head(data.categories)
    p <- ggvis(data.categories, props(x = ~Food_Category, y = ~Count)) +
      mark_rect(props(width := 10))
  }) %>% bind_shiny("foodCategory")
  }
}
shinyServer(server)